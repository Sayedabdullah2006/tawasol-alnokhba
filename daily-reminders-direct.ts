...
function shouldSendReminder(data: ReminderData): boolean {
  ...
  if (!clientWaitingStatuses.includes(status)) return false
  if (data.reminderCount >= REMINDER_SETTINGS.maxReminders[status]) return false
...
function createPaymentReminderHTML(data: ReminderData, reminderNum: number, amount: number): string {
  const messages = [
    ...
    { emoji: '⏰', title: 'نحن منتظرون!', content: `فريقنا متحمس لبدء العمل على مشروعك! إتمام الدفع سيضعك في مقدمة قائمة التنفيذ، وستحصل على مشروعك في أسرع وقت ممكن.` },
    { emoji: '💬', title: 'هل تحتاج مساعدة؟', content: `إذا واجهت أي مشكلة تقنية أو تحتاج مساعدة، فريق دعم العملاء جاهز لمساعدتك خطوة بخطوة.` }
  ]
...
function createContentReminderHTML(data: ReminderData, reminderNum: number): string {
  const messages = [
    ...
    { emoji: '⏰', title: 'في انتظار مراجعتك', content: `نتفهم انشغالك، ولكن مراجعتك مهمة جداً لنا لضمان جودة العمل النهائي. قم بمراجعة النصوص والتصاميم المرفقة.` },
    { emoji: '💬', title: 'هل المحتوى يحتاج تعديل؟', content: `إذا كان يحتاج تعديلات، فلا تتردد في إخبارنا! وإذا كان كل شيء جيد، فقط اضغط موافق!` }
  ]
...
Deno.serve(async (req) => {
  console.log('=== Daily Reminders Job Started ===', new Date().toISOString())
  ...
  try {
    ...
    const { data: requests, error: requestsError } = await supabase
      ...
      .select(`
        id, request_number, client_name, client_email, status,
        created_at, last_status_change, admin_quoted_price, final_total,
        proposed_content, proposed_images
      `)
    ...
    // التحقق من كل طلب وإرسال التذكيرات
    for (const request of requests) {
      try {
        ...
        // إرسال البريد الإلكتروني باستخدام دالة الإرسال المحسنة
