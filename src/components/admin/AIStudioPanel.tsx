...
interface DesignResult {
  ...
  generationRound?: string;
}

type StudioSavedState = {
  analysis?: unknown;
  tweets?: { raw?: string } | null;
  design_concepts?: { items?: ConceptItem[] } | null;
...
type StudioRequest = {
  ...
  ai_design_concepts?: { items?: ConceptItem[] } | null;
...
function normalizeDesignResults(value: unknown): DesignResult[] {
    ...
    return [
      {
        title:
          typeof design.title === "string"
            ...
            : `تصميم ${index + 1}`,
        imageUrl,
        brief: typeof design.brief === "string" ? design.brief : "",
        preparedPrompt:
          typeof design.preparedPrompt === "string"
            ? design.preparedPrompt
            : undefined,
        generationRound:
          typeof design.generationRound === "string"
            ? design.generationRound
            : "legacy",
      },
    ];
  });
...
interface Props {
  ...
  savedStudio?: StudioSavedState; // الحالة المحفوظة لهذا المنشور (ai_posts[postIndex])
}
...
export default function AIStudioPanel({
  ...
  const saved = isPost
    ? {
        ...
        selectedImages: Array.isArray(savedStudio?.selected_images)
          ? savedStudio.selected_images
          : [],
        imageUrl: "", // المعاينة المفردة للجلسة فقط؛ الحفظ عبر designs
        designs: Array.isArray(savedStudio?.designs) ? savedStudio.designs : [],
        uploadedImages: Array.isArray(savedStudio?.uploaded_images)
          ? savedStudio.uploaded_images
          : [],
    ...
    : {
        ...
        uploadedImages: Array.isArray(request.ai_uploaded_images)
          ? request.ai_uploaded_images
          : [],
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const path = createUploadPath("ai-src", file.name);
      ...
      setSelectedImages((prev) => {
        const next = prev.includes(data.publicUrl)
          ? prev
          : [...prev, data.publicUrl];
  ...
  const [selectedImages, setSelectedImages] = useState<string[]>(
    saved.selectedImages.length
      ? saved.selectedImages.filter((url: string) =>
          contentImages.includes(url),
        )
      : saved.sourceImage && contentImages.includes(saved.sourceImage)
        ? [saved.sourceImage]
  ...
  const [analysis, setAnalysis] = useState<unknown>(saved.analysis);
  ...
  const [chosenConcept, setChosenConcept] = useState<string>(
    saved.chosenConcept?.text ?? "",
  );
  const [chosenPreparedPrompt, setChosenPreparedPrompt] = useState<
    string | undefined
  >();
  ...
  const confirmPublish = async () => {
    ...
    if (!cover) return;
    if (!publishText.trim()) {
      showToast("اكتب نص المنشور أولاً", "error");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        showToast(d.error ?? "فشل النشر", "error");
        return;
      }
      ...
      showToast(
        `أُرسل إلى ${n} قناة — تابع الحالة في Post‑Pulse 📣`,
        "success",
      );
    } catch {
  ...
  const featureInMagazine = async (cover: string) => {
    ...
    try {
      ...
      const d = await res.json().catch(() => ({}));
      if (!res.ok) {
        showToast(d.error ?? "تعذّر التضمين في المجلة", "error");
        return;
      }
  ...
  const initialDesigns = normalizeDesignResults(saved.designs);
  const [batchResults, setBatchResults] =
    useState<DesignResult[]>(initialDesigns);
  const roundIds = Array.from(
    new Set(initialDesigns.map((design) => design.generationRound ?? "legacy")),
  );
  const [activeRound, setActiveRound] = useState<string>(
    roundIds.at(-1) ?? "legacy",
  );
  ...
  const [selectedBatch, setSelectedBatch] = useState<Set<number>>(
    new Set(initialDesigns.map((_, i) => i)),
  );
  ...
  const activeRoundResults = batchResults
    ...
    .filter(
      ({ design }) => (design.generationRound ?? "legacy") === activeRound,
    );
  const designRounds = Array.from(
    new Set(batchResults.map((design) => design.generationRound ?? "legacy")),
  );
  ...
  useEffect(() => {
    if ((autoRunning || batchLoading) && batchResults.length > 0) {
      designResultsRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "center",
      });
    }
        ...
        setBatchResults((prev) => {
          const next = [
            ...prev,
            {
              ...
              brief: chosenConcept,
              preparedPrompt: chosenPreparedPrompt,
  ...
  const autoRunAll = async () => {
    ...
    try {
      ...
      const concepts: ConceptItem[] = Array.isArray(c?.concepts)
        ? c.concepts
        : [];
      if (!concepts.length) {
      ...
      for (let i = 0; i < concepts.length; i++) {
        ...
        const url = await generateOneDesign(
          brief,
          undefined,
          concepts[i].imagePrompt,
        );
        if (url) {
          const result = {
            title: concepts[i].title ?? `اتجاه ${i + 1}`,
            imageUrl: url,
            brief,
            preparedPrompt: concepts[i].imagePrompt,
            generationRound: roundId,
          };
          results.push(result);
          setBatchResults((prev) => {
            ...
            setSelectedBatch((selected) =>
              new Set(selected).add(next.length - 1),
            );
            return next;
          });
        }
      ...
      if (results.length)
        showToast(
          `اكتمل التوليد التلقائي — ${results.length} تصاميم جاهزة`,
          "success",
        );
    } catch {
    ...
    try {
      ...
      for (let i = 0; i < conceptItems.length; i++) {
        ...
        const url = await generateOneDesign(
          brief,
          undefined,
          conceptItems[i].imagePrompt,
        );
        if (url) {
          const result = {
            title: conceptItems[i].title ?? `اتجاه ${i + 1}`,
            imageUrl: url,
            brief,
            preparedPrompt: conceptItems[i].imagePrompt,
            generationRound: roundId,
          };
          results.push(result);
          setBatchResults((prev) => {
      ...
      setNoteByIndex({});
      if (results.length)
        showToast(`تم توليد ${results.length} تصاميم`, "success");
    } catch {
  ...
  const regenerateBatchDesign = async (i: number) => {
    ...
    if (!r) return;
    if (!note) {
      showToast("اكتب ملاحظة لإعادة التوليد", "error");
      return;
    }
    setRegenIndex(i);
    try {
  ...
  const editBatchDesign = async (i: number) => {
    ...
    if (!r) return;
    if (!note) {
      showToast("اكتب التعديل المطلوب", "error");
      return;
    }
    setEditIndex(i);
    try {
      ...
      const d = await res.json().catch(() => ({}));
      if (!res.ok) {
        showToast(d.error ?? "فشل التعديل", "error");
        return;
      }
      setBatchResults((prev) => {
        const next = prev.map((x, idx) =>
          idx === i ? { ...x, imageUrl: d.imageUrl } : x,
        );
        persistStudioState({ designs: next });
        return next;
      });
  ...
  const regenerateAllWithNote = async () => {
    ...
    if (!note) {
      showToast("اكتب ملاحظة لإعادة توليد الكل", "error");
      return;
    }
    setBulkBusy(true);
    try {
      ...
      for (let i = 0; i < updated.length; i++) {
        ...
        const url = await generateOneDesign(
          updated[i].brief,
          note,
          updated[i].preparedPrompt,
        );
        if (url) {
          updated[i] = { ...updated[i], imageUrl: url };
          setBatchResults((prev) =>
            prev.map((x, idx) => (idx === i ? { ...x, imageUrl: url } : x)),
          );
          setSelectedBatch((prev) => new Set(prev).add(i));
        }
      }
  ...
  const regenerateSingle = async () => {
    ...
    if (!chosenConcept.trim()) {
      showToast("لا يوجد اتجاه معتمد", "error");
      return;
    }
    setRegenSingle(true);
    try {
  ...
  const cardCls =
    "bg-card rounded-2xl border border-border p-5 space-y-3 shadow-sm";
    ...
    <div className="space-y-4" dir="rtl">
        ...
        {contentImages.length === 0 ? (
          ...
          <>
            <p className="text-xs text-muted">
              اختر صورة أو أكثر لتُضمَّن جميعها في التصميم ويُبنى عليها التحليل
              والاتجاهات:
            </p>
            ...
            {selectedImages.length > 0 && (
              <p className="text-[11px] text-green-700">
                عدد الصور المحددة: {selectedImages.length}
              </p>
            )}
          </>
        )}
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <p className="text-xs text-muted mt-0.5">
              ينفّذ الخطوات تباعاً: تحليل ← تغريدات ← اتجاهات ← تصميم كل
              الاتجاهات.
            </p>
          </div>
          <Button
          ...
          <summary className="list-none cursor-pointer p-4 flex items-center gap-2 hover:bg-cream/40">
            <div className="flex-1">
              <StepHead
                n={1}
                title="تحليل الخبر"
                subtitle="استخراج عناصر الخبر لبناء المحتوى"
                done={!!analysis}
              />
            </div>
            <span className="w-7 h-7 rounded-full border border-green/30 text-green flex items-center justify-center font-bold group-open:rotate-45 transition-transform">
              +
            </span>
          </summary>
          ...
          <summary className="list-none cursor-pointer p-4 flex items-center gap-2 hover:bg-cream/40">
            <div className="flex-1">
              <StepHead
                n={2}
                title="التغريدات"
                subtitle="٣ تغريدات بزوايا ومطالع متنوّعة"
                done={!!tweets}
              />
            </div>
            <span className="w-7 h-7 rounded-full border border-green/30 text-green flex items-center justify-center font-bold group-open:rotate-45 transition-transform">
        ...
        <details className="group bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          <summary className="list-none cursor-pointer p-4 flex items-center gap-2 hover:bg-cream/40">
            <div className="flex-1">
              <StepHead
                ...
                subtitle="٣ اتجاهات مختلفة بناءً على الخبر والصورة"
                done={conceptItems.length > 0}
              />
            </div>
            <span className="w-7 h-7 rounded-full border border-green/30 text-green flex items-center justify-center font-bold group-open:rotate-45 transition-transform">
              +
          ...
          <div className="border-t border-border p-4 space-y-3">
                  ...
                  return (
                    <div
                      ...
                      className={`rounded-xl border p-3 text-xs space-y-1 transition-all ${
                        isChosen
                          ? "border-green ring-2 ring-green/30 bg-green/5"
                    ...
                    >
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setChosenConcept(brief);
                          setChosenPreparedPrompt(c.imagePrompt);
                        }}
                        className={`mt-1 w-full rounded-lg py-1 text-[11px] font-bold transition-colors ${
                          isChosen
                            ? "bg-green text-white"
                            : "bg-white border border-border text-dark hover:border-green"
                        }`}
                      >
            ...
            <textarea
              value={chosenConcept}
              onChange={(e) => {
                setChosenConcept(e.target.value);
                setChosenPreparedPrompt(undefined);
              }}
      ...
      <div className={cardCls}>
        <StepHead
          n={4}
          title="توليد التصميم"
          subtitle="صمّم الاتجاهات الثلاثة أو الاتجاه المعتمد"
          done={batchResults.length > 0 || !!imageUrl}
        />
        ...
        <div className="space-y-2">
          ...
          {activeRoundResults.length > 0 && (
            <div ref={designResultsRef} className="space-y-3">
              <p className="text-xs text-muted">
                اختر التصاميم لإرسالها للعميل، أو اكتب ملاحظة وأعد توليد أي
                تصميم:
              </p>
              {designRounds.length > 1 && (
                <div
                  className="flex gap-2 overflow-x-auto border-b border-border pb-2"
                  aria-label="جولات توليد التصاميم"
                >
                  {designRounds.map((roundId, roundIndex) => {
                    const count = batchResults.filter(
                      (design) =>
                        (design.generationRound ?? "legacy") === roundId,
                    ).length;
                    const generatedRoundNumber =
                      designRounds
                        .filter((id) => id !== "legacy")
                        .indexOf(roundId) + 1;
                    return (
                      <button
                        ...
                        className={`shrink-0 rounded-lg border px-3 py-1.5 text-xs font-bold transition ${activeRound === roundId ? "border-green bg-green text-white" : "border-border bg-white text-dark"}`}
                      >
                        {roundId === "legacy"
                          ? "التصاميم السابقة"
                          : `جولة توليد ${generatedRoundNumber}`}{" "}
              ...
              <div className="rounded-xl border border-green/40 bg-green/5 p-3 space-y-2">
                <p className="text-[11px] text-muted">
                  ✍️ ملاحظة تُطبَّق على كل التصاميم معاً — مثل: احذف نص «كذا»،
                  أضِف «كذا»، أو تعديل عام على التصميم.
                </p>
                <textarea
                  value={bulkNote}
                  onChange={(e) => setBulkNote(e.target.value)}
                  disabled={bulkBusy}
                  placeholder="اكتب ملاحظة لإعادة توليد كل التصاميم..."
                  className="w-full px-3 py-2 rounded-xl border border-border bg-white text-sm min-h-[60px] resize-y"
                />
                <div className="flex items-center gap-2">
                  <Button
                    onClick={regenerateAllWithNote}
                    loading={bulkBusy}
                    disabled={
                      bulkBusy ||
                      regenIndex !== null ||
              ...
              <div className="space-y-3">
                {activeRoundResults.map(({ design: r, index: i }) => {
                  ...
                  return (
                    <div
                      key={i}
                      className={`rounded-xl border-2 p-2 transition-all ${on ? "border-green bg-green/5" : "border-border"}`}
                    >
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0 space-y-1.5">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-dark text-xs truncate">
                              {r.title}
                            </span>
                            <button
                              type="button"
              ...
              <Button
                onClick={() => {
                  const urls = activeRoundResults
                    .filter(({ index }) => selectedBatch.has(index))
                    .map(({ design }) => design.imageUrl);
                  if (!urls.length) {
                    showToast("اختر تصميماً واحداً على الأقل", "error");
                    return;
                  }
                  onUsedContent(
                    selectedTweet,
                    urls,
                    isPost ? (postIndex as number) : 0,
                  );
                }}
                variant="secondary"
                size="sm"
              >
                إرسال التصاميم المختارة (
                {
                  activeRoundResults.filter(({ index }) =>
                    selectedBatch.has(index),
                  ).length
                }
                ) والتغريدة للعميل
              </Button>
        ...
        <div className="border-t border-border pt-3 space-y-2">
          <p className="text-xs text-muted">
            أو صمّم الاتجاه المعتمد فقط (تصميم واحد):
          </p>
          <Button
            ...
            <div className="space-y-3">
              ...
              <div className="bg-cream/60 rounded-xl p-2 space-y-1.5">
                ...
                <Button
                  ...
                  loading={regenSingle}
                  disabled={
                    regenSingle || loadingStep !== null || !singleNote.trim()
                  }
                  variant="outline"
                  size="sm"
                >
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  disabled={publishingCover !== null}
                  variant={
                    publishedCover === imageUrl ? "secondary" : "outline"
                  }
                  size="sm"
                >
                  {publishedCover === imageUrl
                    ? "✅ نُشر"
                    : "📣 انشر عبر القنوات"}
                </Button>
              </div>
        ...
        >
          ...
          >
            <div className="px-5 py-4 border-b border-border">
              ...
              <p className="text-xs text-muted mt-0.5">
                سيُنشر النص أدناه مع التصميم في <b>كل القنوات المربوطة</b> في
                Post‑Pulse فوراً.
              </p>
            </div>
            <div className="px-5 py-4 overflow-y-auto space-y-3">
              ...
              <img
                src={publishCover}
                alt="التصميم"
                className="w-32 mx-auto aspect-[4/5] object-cover rounded-xl border border-border"
              />
              <div>
                <label className="block text-xs font-bold text-dark mb-1">
                  نص المنشور (عدّله قبل النشر):
                </label>
                <textarea
                  value={publishText}
            ...
            <div className="px-5 py-3 border-t border-border flex gap-2">
              <Button
                ...
                loading={publishingCover === publishCover}
                disabled={!!publishingCover || !publishText.trim()}
                className="flex-1"
              >
                🚀 انشر الآن
              </Button>
              <Button
                variant="outline"
                onClick={() => setPublishCover(null)}
                disabled={!!publishingCover}
              >
                إلغاء
              </Button>
            </div>
