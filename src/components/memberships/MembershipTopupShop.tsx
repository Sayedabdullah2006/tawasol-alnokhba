...
export default function MembershipTopupShop({ membership, balances }: Props) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]" dir="rtl">
    ...
    <aside className="h-fit rounded-lg border border-gold/30 bg-dark p-5 text-white shadow-lg lg:sticky lg:top-24">
      <p className="text-xs font-bold text-gold">ملخص العملية</p><h2 className="mt-1 text-xl font-black">{pricing.item.label}</h2><div className="mt-5 space-y-3 border-y border-white/10 py-4 text-sm"><div className="flex justify-between"><span className="text-white/60">الكمية</span><strong>{quantity}</strong></div><div className="flex justify-between"><span className="text-white/60">سعر الوحدة</span><strong>{formatNumber(pricing.item.unitPrice)} ر.س</strong></div><div className="flex justify-between"><span className="text-white/60">قبل الضريبة</span><strong>{formatNumber(pricing.subtotal)} ر.س</strong></div><div className="flex justify-between"><span className="text-white/60">ضريبة القيمة المضافة 15%</span><strong>{formatNumber(pricing.vatAmount)} ر.س</strong></div></div><div className="my-5 flex items-end justify-between"><span className="text-sm text-white/70">الإجمالي</span><strong className="text-2xl text-gold">{formatNumber(pricing.total)} ر.س</strong></div><Button onClick={proceed} loading={submitting} className="w-full !bg-gold !text-dark">متابعة إلى الدفع</Button><p className="mt-3 text-[11px] leading-5 text-white/55">تضاف الوحدات تلقائياً بعد تأكيد الدفع، وتبقى صالحة حتى {new Date(membership.ends_at).toLocaleDateString('ar-SA')}.</p>
    </aside>
  </div>
}
