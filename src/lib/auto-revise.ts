...
async function analyzeRevision(args: {
  textFeedback: string;
  ...
  const fallback: RevisionPlan = {
    summary:
      [
        args.textFeedback && "تعديل النص",
        args.designFeedback && "تعديل التصميم المختار",
      ]
        .filter(Boolean)
        .join("، ") || "مراجعة العميل",
      ...
      messages: [
        {
          role: "system",
          content:
            "أنت محلل ملاحظات مراجعة محتوى. افصل بدقة بين تعديل نص المنشور وتعديل التصميم. لا تخترع طلبات جديدة. أعد JSON فقط بالمفاتيح summary وtextInstruction وdesignInstruction.",
        },
    ...
    return {
      summary:
        typeof parsed.summary === "string" && parsed.summary.trim()
          ...
          : fallback.summary,
      textInstruction:
        typeof parsed.textInstruction === "string"
          ? parsed.textInstruction.trim()
          : fallback.textInstruction,
      designInstruction:
        typeof parsed.designInstruction === "string"
          ? parsed.designInstruction.trim()
          : fallback.designInstruction,
    };
  } catch {
    return fallback;
  }
...
async function rewritePostCopy(
  originalContent: string,
  instruction: string,
): Promise<string> {
      ...
      messages: [
        {
          role: "system",
          content:
            "أنت محرر محتوى عربي محترف. طبّق فقط تعديل العميل على النص. حافظ على الحقائق والأسماء والوسوم الصحيحة، وأعد النص النهائي فقط بلا شرح أو عناوين.",
        },
        {
          role: "user",
        ...
        imageUrl:
          typeof design.imageUrl === "string"
            ...
            : typeof design.url === "string"
              ? design.url
              : undefined,
        brief: typeof design.brief === "string" ? design.brief : undefined,
        revised: Boolean(design.revised),
      };
    })
    .filter((d) => Boolean(d.imageUrl) && !d.revised); // نعدّل من الأصلية فقط
}

/** ينفّذ التعديل الدقيق بالملاحظة على كل تصميم مُرسل. آمن للاستدعاء fire-and-forget. */
export async function autoReviseFromFeedback(args: {
  requestId: string;
  ...
  const referenceImages = Array.isArray(args.referenceImages)
    ? args.referenceImages.filter(Boolean).slice(0, 5)
    : [];
  if (!feedback) return;
  ...
  const { data: req } = await sc
    ...
    .select(
      "id, request_number, client_name, ai_designs, ai_posts, post_reviews",
    )
    .eq("id", requestId)
    .single();
  if (!req) return;
  ...
  const reviews = isRecord(req.post_reviews)
    ? (req.post_reviews as Record<string, ReviewEntry>)
    : {};
  const proposedImages = reviews[postIndex ?? 0]?.proposed_images;
  const sentImages: string[] | undefined = Array.isArray(proposedImages)
    ? proposedImages.filter(
        (image): image is string => typeof image === "string",
      )
    : undefined;

  const aiPostSource = isRecord(req.ai_posts)
    ? (req.ai_posts as Record<string, StudioEntry>)
    : {};
  const entry = isPost ? (aiPostSource[String(postIndex)] ?? {}) : {};
  const studioDesigns = normalizeDesigns(
    isPost ? entry.designs : req.ai_designs,
  );
  ...
  const baseDesigns: DesignEntry[] = sentImages?.length
    ? sentImages.map((url, i) => ({
        title: studioDesigns[i]?.title ?? `تصميم ${i + 1}`,
        imageUrl: url,
      }))
    : studioDesigns;
  ...
  const selectedDesigns =
    args.selectedImage &&
    baseDesigns.some((design) => design.imageUrl === args.selectedImage)
      ? baseDesigns.filter((design) => design.imageUrl === args.selectedImage)
      : baseDesigns;
  const originalContent = String(
    reviews?.[postIndex ?? 0]?.proposed_content ?? "",
  );
  ...
  const revisedText = plan.textInstruction
    ? await rewritePostCopy(originalContent, plan.textInstruction)
    : "";
  ...
  try {
    ...
    for (const d of plan.designInstruction ? selectedDesigns : []) {
      try {
        const { imageUrl } = await editDesign({
          designImageUrl: d.imageUrl as string,
          note: plan.designInstruction,
          referenceImageUrls: referenceImages,
        });
        revised.push({ title: `🔁 معدّل: ${d.title ?? "تصميم"}`, imageUrl });
      } catch {
        /* تجاهل فشل تصميم واحد */
      }
    }
    const payload = {
      feedback,
      text_feedback: args.textFeedback ?? null,
      design_feedback: args.designFeedback ?? null,
      analysis: plan.summary,
      revised_text: revisedText || null,
      revision_base_image: args.selectedImage ?? null,
      reference_images: referenceImages,
      at: new Date().toISOString(),
      designs: revised,
    };
    if (isPost) {
      const aiPosts: Record<string, StudioEntry> = { ...aiPostSource };
      ...
      await sc
        .from("publish_requests")
        .update({ ai_revised_designs: payload })
        .eq("id", requestId);
    }

    const reqNumber = req.request_number
      ? `#${req.request_number}`
      : `#${requestId.slice(0, 8)}`;
    const thumbs = revised
      .map(
        (d) =>
          `<img src="${d.imageUrl}" style="width:110px;border-radius:8px;border:1px solid #e2e8f0;margin:4px" />`,
      )
      .join("");
    await sendEmail(
      ADMIN_EMAIL,
      `🔁 ملاحظات عميل + تصاميم معدّلة — الطلب ${reqNumber}`,
    ...
    console.error(
      `[AUTO_REVISE] فشل إعادة التوليد للطلب ${requestId}:`,
