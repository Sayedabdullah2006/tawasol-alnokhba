...
function isConfirmedPaid(r: AnalyticsRequest): boolean {
  return (
    (r.paid_at != null || PAID_STATUSES.includes(r.status)) &&
    (r.final_total ?? 0) > 0
  );
}
...
function getNetPaidAmount(r: AnalyticsRequest): number {
...
function MonthlyBarChart({
  ...
  return (
    <div className="relative h-44 pt-3">
      <div
        aria-hidden
        className="absolute inset-x-0 top-3 h-32 border-y border-border/70"
      >
        <div className="absolute inset-x-0 top-1/2 border-t border-border/60" />
      </div>
      <div
        className="relative grid h-32 items-end gap-1"
        style={{
      ...
      >
        {data.map((point) => (
          <div
            ...
            title={`اليوم ${point.day}: ${valueLabel(point.value)}`}
          >
            <div
              className="w-full min-h-[3px] rounded-t-sm transition-[height] duration-300 group-hover:opacity-75"
              style={{
                height: `${Math.max((point.value / max) * 100, point.value > 0 ? 4 : 1)}%`,
      ...
      >
        {data.map((point) => (
          <span
            key={point.day}
            className="min-w-0 text-center text-[9px] text-muted"
          >
            {labelDays.has(point.day) ? point.day : ""}
          </span>
        ))}
      </div>
    </div>
...
export default function AdminStatsPage() {
  ...
  const [topCategories, setTopCategories] = useState<
    { category: string; nameAr: string; count: number }[]
  >([]);
  // كل الطلبات — لحساب إحصاءات الشهر المختار ديناميكياً (تتبع فلتر الشهر)
  const [allRequests, setAllRequests] = useState<AnalyticsRequest[]>([]);
  const [funnel, setFunnel] = useState<
    { label: string; count: number; pctOfPrev: number; pctOfTotal: number }[]
  >([]);
  ...
  const [monthlyRevenueByKey, setMonthlyRevenueByKey] = useState<
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const paidCount = requests.filter((r) =>
        paidStatuses.includes(r.status),
      ).length;
      const freeCount = finalTotal - paidCount;

      const monthPaidCount = requests.filter(
        (r) => paidStatuses.includes(r.status) && r.created_at >= monthStart,
      ).length;
      ...
      for (const r of requests) {
        ...
        if (isConfirmedPaid(r)) {
          revenueByMonth[key] =
            (revenueByMonth[key] ?? 0) + getNetPaidAmount(r);
        }
      }
      ...
      setStats({
        ...
        outstanding: requests
          ...
          .reduce(
            (s, r) => s + (r.final_total ?? r.admin_quoted_price ?? 0),
            0,
          ),
      ...
      const stages = [
        { label: "تم استقبال الطلب", count: requests.length },
        {
          ...
          count: requests.filter((r) => r.quoted_at).length,
        },
        {
          label: "اعتمد العميل",
          count: requests.filter((r) => r.approved_at).length,
        },
        {
          label: "تم الدفع",
          count: requests.filter((r) => paidLikeStatuses.includes(r.status))
            .length,
        },
        {
          label: "اكتمل الطلب",
  ...
  const monthStats = useMemo(() => {
    ...
    const inMonth = (r: AnalyticsRequest) => {
    ...
    const byMethod = PAY_METHOD_META.map((mm) => ({
      ...mm,
      amount: agg[mm.key].amount,
      count: agg[mm.key].count,
    }));
  ...
  const monthlyTrend = useMemo(() => {
    ...
    const orders = Array.from({ length: dayCount }, (_, index) => ({
      day: index + 1,
      value: 0,
    }));
    const revenue = Array.from({ length: dayCount }, (_, index) => ({
      day: index + 1,
      value: 0,
    }));
  ...
  const selectedMonthName = selectedDate.toLocaleString("ar", {
    month: "long",
    calendar: "gregory",
  });
  ...
  const monthsFromBaseline =
    (selYear - TARGET_BASELINE_YEAR) * 12 +
    (selMonth - 1 - TARGET_BASELINE_MONTH);
  ...
  const avgOrderRevenue =
    stats.paidCount > 0 ? stats.totalPaidRevenue / stats.paidCount : 0;
  const remainingRevenue = Math.max(0, monthlyTarget - selectedRevenue);
  const ordersNeeded =
    avgOrderRevenue > 0 ? Math.ceil(remainingRevenue / avgOrderRevenue) : null;
  const goalMet = selectedRevenue >= monthlyTarget;

  const isCurrentMonth = selectedMonthKey === initialMonthKey;

  const freePercent =
    stats.total > 0 ? (stats.freeCount / stats.total) * 100 : 0;
  const paidPercent =
    stats.total > 0 ? (stats.paidCount / stats.total) * 100 : 0;
      ...
      <div className="p-4 md:p-6">
        <div className="text-center py-20">
          ...
          <p className="text-muted">
            لا توجد بيانات بعد — ستظهر الإحصائيات عند استلام الطلبات
          </p>
        </div>
      </div>
    );
    ...
    <div className="mx-auto max-w-7xl space-y-5 p-4 md:p-6">
      ...
      <div className="hidden">
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-5xl font-black text-dark mt-1 leading-none">
              {stats.total}
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-cream rounded-xl p-4">
            ...
            <p className="text-xs text-muted mt-1">
              {freePercent.toFixed(1)}% من الإجمالي
            </p>
          </div>
          <div
          ...
          >
            ...
            <p className="text-xs text-muted mt-1">
              {paidPercent.toFixed(1)}% من الإجمالي
            </p>
          </div>
        </div>
      </div>
      ...
      <section className="glass-panel rounded-lg p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-border/70 pb-4">
          <div>
            ...
            <h2 className="mt-1 text-lg font-black text-dark">
              حركة الطلبات والإيرادات
            </h2>
            <p className="mt-1 text-xs text-muted">
              توزيع يومي للطلبات المستلمة والإيرادات المؤكدة.
            </p>
          </div>
          <label className="min-w-[190px] text-xs font-bold text-dark">
            الشهر
            <select
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <article className="rounded-lg border border-border/80 bg-white/40 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-sm font-black text-dark">الطلبات</p>
                <p className="mt-1 text-xs text-muted">
                  إجمالي الشهر:{" "}
                  {monthlyTrend.orders.reduce(
              ...
              <span className="grid h-9 w-9 place-items-center rounded-lg bg-green/10 text-lg text-green">
                ▥
              </span>
            </div>
            <MonthlyBarChart
          ...
          <article className="rounded-lg border border-border/80 bg-white/40 p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="grid h-9 w-9 place-items-center rounded-lg bg-gold/15 text-lg text-gold">
                ⌁
              </span>
            </div>
            <MonthlyBarChart
              ...
              valueLabel={(value) => `${formatNumber(value)} ر.س`}
            />
          </article>
        </div>
      </section>
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          <div className="flex items-start justify-between mb-4">
            <p className="text-sm text-muted leading-tight">
              الطلبات المدفوعة
              <br />
              {isCurrentMonth ? "هذا الشهر" : selectedMonthName}
            </p>
            <div className="text-2xl">📅</div>
          </div>
          <p className="text-4xl font-black text-dark">{monthStats.count}</p>
          <p className="text-xs text-muted mt-2">
            طلب مدفوع في {selectedMonthName} {selYear}
          </p>
            ...
            <div className="mt-4">
              <div className="h-2 bg-cream rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${Math.min((monthStats.count / Math.max(stats.paidCount, 1)) * 100, 100)}%`,
                  }}
                />
              </div>
              <p className="text-xs text-muted mt-1">
                {(
                  (monthStats.count / Math.max(stats.paidCount, 1)) *
                  100
                ).toFixed(0)}
                % من إجمالي المدفوعة
              </p>
          ...
          <div className="mt-4 pt-4 border-t border-border">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-bold text-dark">
                توزيع مبالغ {isCurrentMonth ? "الشهر" : selectedMonthName} على
                طرق الدفع
              </p>
              <p className="text-xs font-black text-green">
                {formatNumber(
                  monthStats.byMethod.reduce((s, m) => s + m.amount, 0),
                )}{" "}
                ر.س
              </p>
            </div>
            {(() => {
              const monthTotal = monthStats.byMethod.reduce(
                (s, m) => s + m.amount,
                0,
              );
              if (monthTotal <= 0) {
                return (
                  <p className="text-xs text-muted">
                    لا مدفوعات مؤكّدة في {selectedMonthName} بعد.
                  </p>
                );
              }
              return (
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          <div className="flex items-start justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-muted opacity-70 mt-0.5">
                {formatNumber(monthlyTarget)} ر.س · يزيد 5% كل شهر
              </p>
            </div>
            <div className="text-2xl">🎯</div>
          </div>
          ...
          >
            {availableMonths.map((key) => {
              ...
              const label = new Date(y, m - 1, 1).toLocaleString("ar", {
                month: "long",
                year: "numeric",
                calendar: "gregory",
              });
          ...
          <div className="flex items-end gap-2 mb-0.5">
            ...
            {(prevRevenue > 0 || selectedRevenue > 0) && (
              <span
                className={`text-sm font-bold mb-0.5 ${isRevenueUp ? "text-green" : "text-red-500"}`}
              >
          ...
          <p className="text-xs text-muted mb-4">
            {selectedMonthName} {selYear}
            {isCurrentMonth ? " (هذا الشهر)" : ""}
          </p>
          ...
          <div className="mb-3">
            <div className="flex justify-between text-xs text-muted mb-1.5">
              ...
              <span
                className={`font-bold ${goalAchieved >= 100 ? "text-green" : "text-dark"}`}
              >
          ...
          {monthlyTarget > 0 && (
            <div
              className={`mt-3 rounded-xl p-3 text-center text-xs ${goalMet ? "bg-green/10" : "bg-amber-50"}`}
            >
              {goalMet ? (
                <p className="font-bold text-green">
                  تحقق الهدف في {selectedMonthName}!
                </p>
              ) : (
                <>
                  ...
                  {avgOrderRevenue > 0 && (
                    <p className="text-muted mt-0.5">
                      بمعدل {formatNumber(Math.round(avgOrderRevenue))} ر.س /
                      طلب
                    </p>
                  )}
                </>
        ...
        {[
          {
            ...
            value: `${formatNumber(stats.revenue)} ر.س`,
            icon: "💰",
          },
          {
            label: `إيرادات ${isCurrentMonth ? "هذا الشهر" : selectedMonthName}`,
            value: `${formatNumber(monthStats.revenue)} ر.س`,
            icon: "💵",
          },
          {
            label: "عروض بانتظار الموافقة",
            value: `${formatNumber(stats.outstanding)} ر.س`,
            icon: "🧾",
          },
          {
          ...
          >
            ...
            <div className="text-base font-black text-dark break-words leading-tight">
              {s.value}
            </div>
      ...
      {funnel.length > 0 && funnel[0].count > 0 && (
        <div className="bg-card rounded-2xl border border-border p-5">
          ...
          <p className="text-xs text-muted mb-4">
            نسبة الانتقال من كل مرحلة للتالية — اكتشف أين يتسرّب العملاء.
          </p>
              ...
              <div key={f.label}>
                <div className="flex items-center justify-between text-sm mb-1">
                  ...
                  <span className="text-muted text-xs">
                    ...
                    {i > 0 && (
                      <span
                        className={`ms-2 ${f.pctOfPrev >= 80 ? "text-green" : f.pctOfPrev >= 50 ? "text-orange-600" : "text-red-600"}`}
                      >
