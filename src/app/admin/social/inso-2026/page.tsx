          ...
          {savedDayItems.length ? <div className="space-y-3">
            {savedDayItems.map(item => {
              ...
              return <article key={item.id} className="rounded-lg border border-teal-200 bg-teal-50/30 p-4">
                ...
                {item.design_options?.length ? <div className="mt-4 flex gap-3 overflow-x-auto pb-2 sm:grid sm:grid-cols-3 sm:overflow-visible">{item.design_options.map(option => <div key={option.id} className={`w-40 shrink-0 overflow-hidden rounded-lg border sm:w-auto ${option.selected ? 'border-teal-500 bg-teal-50/40' : 'border-border bg-white'}`}>
                  <button type="button" onClick={() => setDesignPreview({ title: `${item.title} - ${option.title}`, imageUrl: option.imageUrl })} className="relative block w-full" title="تكبير التصميم" aria-label={`تكبير ${option.title}`}>
                    ...
                    <img src={option.imageUrl} alt={`${item.title} - ${option.title}`} className="h-48 w-full object-cover sm:h-auto sm:aspect-[4/5]" />
                    <span className="absolute bottom-2 left-2 rounded-md bg-black/70 px-2 py-1 text-[11px] font-bold text-white">تكبير ⤢</span>
                  </button>
                  <div className="space-y-2 p-2"><p className="text-xs font-bold text-dark">{option.title}</p><p className="hidden text-[11px] text-muted sm:block">{option.direction}</p><div className="grid grid-cols-2 gap-1"><Button size="sm" className="w-full" onClick={() => callAction('select-design-option', { id: item.id, optionId: option.id }, 'تم اعتماد التصميم')} variant={option.selected ? 'secondary' : 'outline'}>{option.selected ? 'معتمد' : 'اعتماد'}</Button><Button size="sm" className="w-full" variant="ghost" onClick={() => { setEditOption({ item, optionId: option.id }); setEditNote('') }}>تعديل</Button></div></div>
                </div>)}</div> : null}
              </article>
            })}
