    ...
    <main className="p-4 md:p-6 max-w-5xl mx-auto space-y-5" dir="rtl">
      ...
      <div className="space-y-3">
        ...
        {items.map(item => <article key={item.id} className="border border-border bg-card p-4 rounded-lg space-y-3">
          <div className="flex flex-wrap justify-between gap-2"><div><a href={item.post_url} target="_blank" rel="noreferrer" className="font-bold text-dark hover:text-green">@{item.author_username ?? item.author_name ?? 'حساب موثق'}</a><span className="text-xs text-muted mr-2">{item.source_type === 'verified_reply_to_first1' ? 'رد موثق على أول سعودي' : 'منشور موثق مرتبط بالاهتمامات'}</span></div><span className={`text-xs font-bold ${item.status === 'published' ? 'text-green' : 'text-dark'}`}>{item.status === 'published' ? 'تم النشر' : `ملاءمة ${item.relevance_score}%`}</span></div>
