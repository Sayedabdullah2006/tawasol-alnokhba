...
export default function MembershipTopupPaymentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-3xl px-4 py-8" dir="rtl">
    ...
    <section className="overflow-hidden rounded-lg border border-border bg-card shadow-lg">
      ...
      <div className="p-5"><div className="mb-5 grid gap-3 rounded-lg bg-cream p-4 text-sm sm:grid-cols-3"><div><p className="text-muted">الميزة</p><p className="font-black text-dark">{item?.shortLabel}</p></div><div><p className="text-muted">الكمية</p><p className="font-black text-dark">{topup.quantity}</p></div><div><p className="text-muted">الإجمالي شامل الضريبة</p><p className="font-black text-gold">{formatNumber(Number(topup.total_amount))} ر.س</p></div></div>
