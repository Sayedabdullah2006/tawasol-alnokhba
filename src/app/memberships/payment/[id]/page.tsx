...
export default function MembershipPaymentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-4xl px-4 py-8" dir="rtl">
    ...
    <section className="overflow-hidden rounded-lg border border-border bg-card shadow-lg">
      ...
      <div className="p-5">
        <div className="mb-5 grid grid-cols-2 gap-3 rounded-lg bg-cream p-4 text-sm"><div><p className="text-muted">رصيد النشر طوال المدة</p><p className="font-black text-dark">{membership.membership_plan_prices?.included_credits} رصيد</p></div><div><p className="text-muted">الإجمالي شامل الضريبة</p><p className="font-black text-gold">{formatNumber(amount)} ر.س</p></div></div>
